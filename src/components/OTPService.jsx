...
// For demo purposes, we define a simple Link component, as React Router is not available.
 ...
 * * NOTE: The original context was for Email Verification, but this component is 
 * structured for Phone Number verification based on the 'phoneNumber' prop.
 * * @param {Object} props
 * @param {string} props.phoneNumber - The phone number (or email) to send OTP to
...
const OTPVerification = ({
	...
	const handleSendOtp = async () => {
		...
		try {
			...
			setError(err.message || 'Failed to send OTP. Please check the number/email.');
		} finally {
			setIsLoading(false);
		}
	};
	...
	const handleVerifyOtp = async () => {
		// Ensure OTP is at least 4 digits (common minimum for OTP codes)
		...
		try {
			// Pass OTP and phone/email to the verification API
			...
			<div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 transform transition-all scale-100 ease-out duration-300">
				...
				<div className="mb-6 text-center">
					...
					<h2 className="text-2xl font-bold text-gray-900 mb-2">
						{isVerified ? 'Verification Successful!' : 'Verify Your Identity'}
					</h2>
					<p className="text-gray-600 text-sm">
						{isSent ? `Enter the 6-digit code sent to ${phoneNumber}` : 'Sending verification code...'}
					</p>
				</div>
...
const ExampleUsage = () => {
	...
	// NOTE: In a real app, these functions would handle token/email logic from the initial enrollment flow.
		...
		<div className="min-h-screen bg-gray-100 p-8 font-sans">
			<div className="max-w-3xl mx-auto">
				...
				<div className="bg-white rounded-xl p-8 shadow-xl mb-6">
					...
					<p className="text-gray-600 mb-4">
						...
						The mock API will auto-send the code on mount. For successful verification,
						use the code <code className="bg-green-100 text-green-800 px-2 py-0.5 rounded font-mono font-semibold text-sm">123456</code>.
					</p>
					<div className="flex items-center justify-between">
						<p className="text-sm font-medium text-gray-500">
							Current Status: <span className={`font-semibold ${status.includes('Sent') ? 'text-blue-600' : status.includes('Success') ? 'text-green-600' : 'text-gray-900'}`}>{status}</span>
						</p>
						<button
							onClick={() => setShowOtp(true)}
							className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-colors shadow-lg"
						>
				...
				<div className="bg-white rounded-xl p-8 shadow-xl">
					...
					<pre className="bg-gray-100 p-4 rounded-lg overflow-x-auto text-sm border border-gray-200">
						{`<OTPVerification
							phoneNumber="234803XXXXXXX" // The destination (phone or email)
							redirectUrl="/dashboard"     // The final destination URL
							onSuccess={(result) => { 
								// Handle token storage, local state update, etc.
								console.log('Final success action:', result.token);
							}}
			...
			{showOtp && (
				<OTPVerification
					...
					onSuccess={(result) => {
						setStatus(`SUCCESS: Redirecting with token: ${result.token}`);
						// In a real app, this is where you'd save the token (e.g., in a cookie/localStorage)
						setShowOtp(false);
					}}
					onCancel={() => {
